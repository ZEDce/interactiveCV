...
export function parseCvMarkdown(markdown: string): CvData {
  ...
  // --- Contact ---
  const contactSection = extractSection(
    "**Kontakt:**",
    "**Profil / Zhrnutie:**"
  );
  if (contactSection) {
    ...
    lines.forEach((line) => {
      if (line.startsWith("-   Telefón:"))
        data.contact.phone = line.substring(13).trim();
      else if (line.startsWith("-   Email:"))
        data.contact.email = line.substring(11).trim();
      else if (line.startsWith("-   Lokalita:"))
        data.contact.location = line.substring(14).trim();
      else if (line.startsWith("-   LinkedIn:"))
        data.contact.linkedIn = line.substring(14).trim();
      else if (line.startsWith("-   Portfólio:"))
        data.contact.portfolio = line.substring(15).trim();
    });
  }

  // --- Summary ---
  data.summary = extractSection(
    "**Profil / Zhrnutie:**",
    "**Pracovné Skúsenosti:**"
  ).replace(/\*\*/g, "");
    ...
    lines.forEach((line) => {
      if (line.startsWith("**") && line.includes("|")) {
        ...
        currentExperience.details.push(
          line.substring(4).trim().replace(/\*\*/g, "")
        );
  ...
  // --- Certificates ---
  const certificatesSection = extractSection(
    "**Certifikáty:**",
    "**Projekty:**"
  );
    ...
    lines.forEach((line) => {
      if (line.startsWith("-   ")) {
        ...
        const name = dateMatch
          ? item.substring(0, dateMatch.index).trim().replace(/\*\*/g, "")
          : item.replace(/\*\*/g, "");
  ...
  if (projectsSection) {
    ...
    const lines = projectsSection
      .split("\n")
      .map((l) => l.trim())
      .filter((l) => l); // Split and remove empty lines
      ...
      if (line.startsWith("- **")) {
        ...
        if (titleEndIndex !== -1) {
          ...
          const description = line
            .substring(titleEndIndex + titleEndMarker.length)
            .trim(); // Extract text after ':**'
    ...
    if (
      ...
      !data.projects.includes(currentProject)
    ) {
      data.projects.push(currentProject);
    }
  }
  ...
  if (skillsSection) {
    ...
    lines.forEach((line) => {
      ...
      if (line.startsWith(startMarker) && colonIndex > startMarker.length) {
        ...
        if (skills.length > 0) {
          // Only push if there are actual skills listed
    ...
    for (const line of lines) {
      if (line.startsWith("- ")) {
        ...
        data.interests = interestsString
          ...
          .filter((interest) => interest); // Split, trim, filter empty
        break; // Assume only one line matters for interests
      }
    }
    if (!data.interests || data.interests.length === 0) {
      console.warn(
        "CV Parser: Could not parse interests from line starting with '- '"
      );
      ...
      const fallbackInterests = interestsSection
        .split(",")
        .map((i) => i.trim())
        .filter((i) => i);

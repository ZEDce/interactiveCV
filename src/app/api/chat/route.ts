...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!userQuery || !userId) {
      ...
      return NextResponse.json(
        { error: "Query or userId is missing" },
        { status: 400 }
    ...
    if (!n8nResponse.ok) {
      ...
      console.error(
        `n8n webhook request failed with status ${n8nResponse.status}: ${errorText}`
      );
    ...
    return NextResponse.json(
      { error: "Failed to process chat request due to an internal error." },
      { status: 500 }
    );
  }
}
